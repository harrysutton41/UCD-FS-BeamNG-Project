%% Batch 2 PCA figure generation script
...
%   - configs.xlsx
%   - batch2_ai_summary.csv
%   - batch2_manual_summary.csv
%
% Outputs are saved to: batch2_pca_figures_whitejet_outputs
...
batchLabel = 'Batch 2';
configSheet = 2;
aiFile = 'batch2_ai_summary.csv';
manualFile = 'batch2_manual_summary.csv';
outFolder = fullfile(pwd, 'batch2_pca_figures_whitejet_outputs');
...
grid on; hold on;
for i = 1:height(TAI)
    text(TAI.PC1(i)+0.02, TAI.PC2(i), TAI.config_id{i}, 'FontSize', 9, 'Interpreter', 'none');
end
...
grid on; hold on;
for i = 1:height(TMAN)
    text(TMAN.PC1(i)+0.02, TMAN.PC2(i), TMAN.config_id{i}, 'FontSize', 9, 'Interpreter', 'none');
end
...
grid on;
for i = 1:height(TAI)
    text(TAI.PC1(i)+0.02, TAI.PC2(i), ['AI ' TAI.config_id{i}], 'FontSize', 8, 'Interpreter', 'none');
end
for i = 1:height(TMAN)
    text(TMAN.PC1(i)+0.02, TMAN.PC2(i)-0.04, ['MAN ' TMAN.config_id{i}], 'FontSize', 8, 'Interpreter', 'none');
end
